...
def main():
    ...
    parser.add_argument("--read_count_bacteria", type=int, default=0, help="Minimum read count threshold for bacteria.")
    parser.add_argument("--read_count_virus", type=int, default=0, help="Minimum read count threshold for viruses.")
    parser.add_argument("--top_N_bacteria", type=int, default=None, help="Select the top N most common bacteria.")
    parser.add_argument("--top_N_virus", type=int, default=None, help="Select the top N most common viruses.")
    ...
    if args.no_metadata:
        ...
        merged_tsv_path = aggregate_kraken_results(args.output_dir, sample_id_df=sample_id_df, read_count=args.read_count_bacteria)  # Use read_count_bacteria for initial aggregation
    else:
        if not args.metadata_file:
            ...
            sys.exit(1)
        merged_tsv_path = aggregate_kraken_results(args.output_dir, metadata_file=args.metadata_file, read_count=args.read_count_bacteria)  # Use read_count_bacteria for initial aggregation
    ...
    if merged_tsv_path and os.path.isfile(merged_tsv_path):
        if args.virus:
            ...
            generate_abundance_plots(merged_tsv_path, args.top_N_virus, read_count=args.read_count_virus)  # Pass read_count_virus
        if args.bacteria:
            logging.info("Generating bacterial abundance plots.")
            generate_abundance_plots(merged_tsv_path, args.top_N_bacteria, read_count=args.read_count_bacteria)  # Pass read_count_bacteria
        if not args.virus and not args.bacteria:
